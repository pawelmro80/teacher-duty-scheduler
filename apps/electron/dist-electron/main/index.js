...
const backendDir = node_path.join(cwd, "../../apps/backend");
const venvPython = isWin ? node_path.join(backendDir, "venv/Scripts/python.exe") : node_path.join(backendDir, "venv/bin/python");
const pythonExec = isWin ? "python" : "python3";
const pythonPath = electron.app.isPackaged ? node_path.join(process.resourcesPath, "python", "main" + (isWin ? ".exe" : "")) : require("fs").existsSync(venvPython) ? venvPython : pythonExec;
function startPythonBackend() {
  ...
  try {
    if (electron.app.isPackaged) {
      ...
      const scriptPath = node_path.join(backendDir, "main.py");
